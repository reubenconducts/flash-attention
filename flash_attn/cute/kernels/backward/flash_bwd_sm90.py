...
from flash_attn.cute.kernels.common import pipeline
...
from flash_attn.cute.kernels.common import barrier
from flash_attn.cute.kernels.common.named_barrier import NamedBarrierBwd
from flash_attn.cute.kernels.common.softmax import apply_score_mod_inner, apply_score_mod_bwd_inner
...
from flash_attn.cute.kernels.backward.flash_bwd import FlashAttentionBackwardSm80, _hdim_sym
...
class FlashAttentionBackwardSm90:
    ...
    # __call__ mirrors FlashAttentionBackwardSm80's signature exactly, so the
    # Args declaration is shared (enforced by check_kernel_contract).
    Args = FlashAttentionBackwardSm80.Args
    ...
    class Params(KernelParams):
        ...
        tile_m: int = sym_expr(lambda p: f"t{p.tile_m}x{p.tile_n}", default=64)
        tile_n: int = no_sym(default=128)
        Q_stage: int = sym_val("qs{}", skip=1, default=2)
        dO_stage: int = sym_val("dos{}", skip=1, default=2)
        PdS_stage: int = sym_val("pdss{}", skip=1, default=2)
        ...
        AtomLayoutNdKV: int = sym_val("atomdkv{}", skip=1, default=2)
        AtomLayoutMdQ: int = sym_val("atomdq{}", skip=1, default=1)
        num_threads: int = sym_val("th{}", default=384)
        ...
        dQ_single_wg: bool = sym_tag("dq_1wg", default=False)

    def __init__(self, params: "FlashAttentionBackwardSm90.Params"):
        ...
        self.dO_stage = params.dO_stage
        self.PdS_stage = params.PdS_stage
        ...
        self.mma_dkv_is_rs = (
            params.AtomLayoutMSdP == 1
            and params.AtomLayoutNdKV == self.num_wg_mma
            and params.SdP_swapAB
            and not params.dKV_swapAB
        )
        self.V_in_regs = params.V_in_regs
        # May be overridden in __call__ for varlen inputs.
        if params.qhead_per_kvhead > 1:
            assert self.same_hdim_kv, "GQA backward requires params.head_dim == head_dim_v"
        ...
        # pressure when params.SdP_swapAB, where each thread needs to keep statistics for (kBlockM / 4)
        # rows. If !params.SdP_swapAB, each thread only needs to keep statistics for 2 rows.
        ...
        self.has_aux_tensors = params.has_aux_tensors
        self.q_subtile_factor = params.q_subtile_factor
        if cutlass.const_expr(params.has_aux_tensors):
            self.vec_size: cutlass.Constexpr = 1
        else:
            self.vec_size: cutlass.Constexpr = 4
        self.qk_acc_dtype = Float32
        # params.dQ_single_wg: WG0 computes the full dQ GEMM, WG1 skips it.
        # Only valid for 2 MMA warp groups.
        # Credit: Ben Spector
        if params.dQ_single_wg:
            assert self.num_wg_mma == 2, "params.dQ_single_wg only supports 2 warp groups"
        self.num_wg_dQ = 1 if params.dQ_single_wg else self.num_wg_mma
