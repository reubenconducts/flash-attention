...
class FlashAttentionBackwardPostprocess:
    ...
    class Params(KernelParams):
        ...
        ``arch`` is the compilation target (may differ from the ambient device
        arch under FLASH_ATTENTION_ARCH); the ambient arch is already keyed and
        named by the runner, so it carries no symbol of its own.
        """
    ...
    class Args(KernelArgs):
        ...
        mdQaccum: torch.Tensor = fa_tensor(
            shape=lambda a, p: ("h", "total_q_dr") if _is_varlen(a, p) else ("b", "h", "s_dr"),
        )
        mdQ: torch.Tensor = fa_tensor(
            shape=lambda a, p: ("total_q", "h", "d") if _is_varlen(a, p) else ("b", "s", "h", "d"),
        )
    ...
    def __init__(self, params: "FlashAttentionBackwardPostprocess.Params"):
        """
        ...
        assert params.arch // 10 in [8, 9, 10, 11, 12], (
        ...
        self.use_2cta_instrs = (
            params.use_2cta_instrs and params.arch // 10 in [10, 11] and params.head_dim != 64
        )
