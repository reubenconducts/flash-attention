...
from flash_attn.cute.kernels.common import copy_utils
from flash_attn.cute.kernels.common import pipeline
from flash_attn.cute.kernels.arch.blackwell_helpers import gemm_w_idx, gemm_ptx_w_idx  # noqa
...
from flash_attn.cute.kernels.common.named_barrier import NamedBarrierBwdSm100
...
class FlashAttentionBackwardSm100:
    ...
    class Params(KernelParams):
        ...
        spt: Optional[bool] = sym_tag("spt", default=None)
    ...
    def __init__(self, params: "FlashAttentionBackwardSm100.Params"):
        ...
        self.use_2cta_instrs = bool(params.use_2cta_instrs and params.cluster_size == 2)
        self.cta_group_size = 2 if self.use_2cta_instrs else 1

        assert self.tile_hdim != 192 or self.use_2cta_instrs, "Must use 2CTA for hdim 192"

        # CTA tiler
        self.cta_tiler = (params.tile_n, params.tile_m, self.tile_hdim)
        # S = K @ Q.T
        self.mma_tiler_kq = (self.cta_group_size * params.tile_n, params.tile_m, self.tile_hdim)
        # dP = V @ dO.T
        self.mma_tiler_vdo = (self.cta_group_size * params.tile_n, params.tile_m, self.tile_hdimv)
        # dV = P.T @ dO
        self.mma_tiler_pdo = (self.cta_group_size * params.tile_n, self.tile_hdimv, params.tile_m)
        # dK = dS.T @ Q
        self.mma_tiler_dsq = (self.cta_group_size * params.tile_n, self.tile_hdim, params.tile_m)
        # dQ = dS @ K
        # 2-CTA: reduction dim is cluster-wide (params.tile_n * cta_group_size).
        self.mma_tiler_dsk = (params.tile_m, self.tile_hdim, params.tile_n * self.cta_group_size)

        self.acc_dtype = Float32

        assert params.cluster_size in (1, 2), "Only cluster_size=1 or 2 is supported"
        self.cluster_shape_mn = (params.cluster_size, 1)
        ...
        # For params.score_mod, use vec_size=1 (like forward) to handle per-element indices
        ...
        if (not params.is_causal and not params.is_local) or params.deterministic:
        ...
        if const_expr(self.tile_hdim == 192):
            if not params.is_causal and not params.is_local:
