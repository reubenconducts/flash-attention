...
from flash_attn.cute.kernels.common.seqlen_info import SeqlenInfo
...
from flash_attn.cute.kernels.common.pack_gqa import pack_gqa_layout


def _is_varlen(args, params) -> bool:
    return args.mCuSeqlensQ is not None


def _o_shape(args, params):
    return ("total_q", "h", "dv") if _is_varlen(args, params) else ("b", "s", "h", "dv")


def _seq_shape(suffix):
    """LSE-family layouts: seqlen-contiguous, heads-minor unless nheads_major.
    ``suffix`` distinguishes the raw / rounded / rounded*head_dim seqlen syms."""

    def shape(args, params):
        seq = ("total_q" if _is_varlen(args, params) else "s") + suffix
        if params.nheads_major:
            return (seq, "h") if _is_varlen(args, params) else ("b", seq, "h")
        return ("h", seq) if _is_varlen(args, params) else ("b", "h", seq)

    return shape


def _dqaccum_shape(args, params):
    # dQaccum requires nheads_major=False (asserted in __call__).
    return ("h", "total_q_dr") if _is_varlen(args, params) else ("b", "h", "s_dr")


def _rowmax_shape(args, params):
    # RowMax/ScaleP require nheads_major=True (asserted in __call__).
    return ("total_q", "topk", "h") if _is_varlen(args, params) else ("b", "s", "topk", "h")


class FlashAttentionBackwardPreprocess:
    KERNEL_NAME = "flash_bwd_pre"
    compile_mode = "symbolic"
    ...
    class Params(KernelParams):
        ...
        head_dim: int = sym_val("d{}")
        head_dim_v: int = sym_val(lambda v: f"dv{v}", skip=lambda p: p.head_dim_v == p.head_dim)
        tile_m: int = sym_val("m{}", default=128)
        num_threads: int = sym_val("th{}", default=256)
        use_padded_offsets: bool = no_sym(default=True)
        nheads_major: bool = sym_tag("hmajor", default=False)
        pack_gqa: bool = sym_tag("pack_gqa", default=False)
        qhead_per_kvhead: int = sym_val("gqa{}", skip=1, default=1)
        nheads_kv: int = no_sym(default=1)

    @dataclass
    class Args(KernelArgs):
        """Runtime __call__ arguments: mirrors __call__ (minus stream) field-for-field.

        Shape templates share syms per compilation: the LSE-family tensors'
        seqlen dim matches mO's ("s"/"total_q"), while the rounded ("_r") and
        rounded*head_dim ("_dr") variants are independent syms.
        """

        mO: torch.Tensor = fa_tensor(shape=_o_shape)
        mdO: torch.Tensor = fa_tensor(shape=_o_shape)
        mPdPsum: torch.Tensor = fa_tensor(shape=_seq_shape("_r"))
        mLSE: Optional[torch.Tensor] = fa_tensor(align=4, shape=_seq_shape(""))
        mLSElog2: Optional[torch.Tensor] = fa_tensor(shape=_seq_shape("_r"))
        mdQaccum: Optional[torch.Tensor] = fa_tensor(shape=_dqaccum_shape, sym="dqacc")
        mCuSeqlensQ: Optional[torch.Tensor] = fa_tensor(
            align=4, shape=("b_p1",), leading_dim=0, sym="varlen"
        )
        mSeqUsedQ: Optional[torch.Tensor] = fa_tensor(
            align=4, shape=("b",), leading_dim=0, sym="seqused"
        )
        mdLSE: Optional[torch.Tensor] = fa_tensor(align=4, shape=_seq_shape(""), sym="dlse")
        mRowMax: Optional[torch.Tensor] = fa_tensor(align=4, shape=_rowmax_shape)
        mScaleP: Optional[torch.Tensor] = fa_tensor(align=4, shape=_rowmax_shape, sym="scalep")
        softmax_scale: float = scalar(Float32)

    @classmethod
    def can_implement_params(cls, params) -> Optional[str]:
        ok = cls.can_implement(params.dtype, params.head_dim, params.tile_m, params.num_threads)
        return None if ok else "unsupported dtype/head_dim/tile/num_threads configuration"

    def __init__(self, params: "FlashAttentionBackwardPreprocess.Params"):
        """
        ...
        :param params.tile_m: m block size
        :type params.tile_m: int
        ...
        # padding params.head_dim to a multiple of 32 as k_block_size
        hdim_multiple_of = 32
        self.head_dim_padded = int(math.ceil(params.head_dim / hdim_multiple_of) * hdim_multiple_of)
        self.head_dim_v_padded = int(
            math.ceil(params.head_dim_v / hdim_multiple_of) * hdim_multiple_of
        )
        self.check_hdim_v_oob = params.head_dim_v != self.head_dim_v_padded
        self.num_threads = params.num_threads
        self.use_padded_offsets = params.use_padded_offsets
        self.nheads_major = params.nheads_major
    ...
    def __call__(
        ...
        mO = args.mO  # (batch, seqlen, nheads, head_dim_v) or (total_q, nheads, head_dim_v)
        mdO = args.mdO  # same shape as mO
        mPdPsum = args.mPdPsum  # (batch, nheads, seqlen_padded) or (nheads, total_q_padded)
        mLSE = args.mLSE  # (batch, nheads, seqlen) or (nheads, total_q)
        mLSElog2 = args.mLSElog2  # same shape as mPdPsum
        # (batch, nheads, seqlen_padded * head_dim_v) or (nheads, total_q_padded * head_dim_v)
        mdQaccum = args.mdQaccum
        mCuSeqlensQ = args.mCuSeqlensQ  # (batch + 1,)
        mSeqUsedQ = args.mSeqUsedQ  # (batch,)
        mdLSE = args.mdLSE  # (batch, nheads, seqlen) or (nheads, total_q)
        mRowMax = args.mRowMax  # (b, s, n, h) or (t, n, h)
        mScaleP = args.mScaleP  # == mRowMax
