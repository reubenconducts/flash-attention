...
from flash_attn.cute.kernels.arch import ampere_helpers as sm80_utils
...
from flash_attn.cute.kernels.common.softmax import call_score_mod, call_score_mod_bwd
...
from flash_attn.cute.kernels.common.tile_scheduler import SingleTileScheduler, SingleTileVarlenScheduler, TileSchedulerArguments
...
def _semaphore_arg(t):
    """Deterministic-mode semaphores keep the last dim static (leading) with the
    remaining strides dynamic; plain to_cute_tensor would not."""
    return utils.convert_from_dlpack_leading_static(
        t.detach(), leading_dim=3, alignment=4, stride_order=t.dim_order()
    )


class FlashAttentionBackwardSm80:
    KERNEL_NAME = "flash_bwd"
    compile_mode = "real"
    ...
    class Params(KernelParams):
        ...
        dtype: Type[cutlass.Numeric] = sym_val(short_dtype_name)
        ...
        m_block_size: int = sym_expr(
            lambda p: f"t{p.m_block_size}x{p.n_block_size}", default=64
        )
        n_block_size: int = no_sym(default=128)
        num_stages_Q: int = sym_val("qs{}", skip=1, default=2)
        num_stages_dO: int = sym_val("dos{}", skip=1, default=2)
        num_threads: int = sym_val("th{}", default=256)
        ...
        SdP_swapAB: bool = sym_tag("sdp_swab", default=False)
        dKV_swapAB: bool = sym_tag("dkv_swab", default=False)
        dQ_swapAB: bool = sym_tag("dq_swab", default=False)
        AtomLayoutMSdP: int = sym_val("atomsdp{}", skip=1, default=1)
        AtomLayoutNdKV: int = sym_val("atomdkv{}", skip=1, default=8)
        AtomLayoutMdQ: int = sym_val("atomdq{}", skip=1, default=1)
        V_in_regs: bool = sym_tag("v_regs", default=False)
    ...
    class Args(KernelArgs):
        ...
        mLSE receives lse*log2(e); mdK/mdV receive the fp32 accumulators
        instead of the outputs when GQA postprocessing is enabled (the dtype
        difference is keyed via spec()). Broadcast patterns of the inputs are
        keyed because stride-0 dims are baked static at compile time.
        """

        mQ: torch.Tensor = fa_tensor(detach=True, keyed_broadcast=True, sym_dtype=True)
        mK: torch.Tensor = fa_tensor(detach=True, keyed_broadcast=True)
        mV: torch.Tensor = fa_tensor(detach=True, keyed_broadcast=True)
        mdO: torch.Tensor = fa_tensor(keyed_broadcast=True)
        mLSE: torch.Tensor = fa_tensor()
        mdPsum: torch.Tensor = fa_tensor()
        mdQaccum: torch.Tensor = fa_tensor()
        mdK: torch.Tensor = fa_tensor()
        mdV: torch.Tensor = fa_tensor()
        softmax_scale: float = scalar(Float32)
        mCuSeqlensQ: Optional[torch.Tensor] = fa_tensor(align=4, sym="varlen", default=None)
        mCuSeqlensK: Optional[torch.Tensor] = fa_tensor(align=4, sym="varlen", default=None)
        mSeqUsedQ: Optional[torch.Tensor] = fa_tensor(align=4, sym="varlen", default=None)
        mSeqUsedK: Optional[torch.Tensor] = fa_tensor(align=4, sym="varlen", default=None)
        window_size_left: Optional[int] = scalar(Int32, default=None)
        window_size_right: Optional[int] = scalar(Int32, default=None)
        mdQ_semaphore: Optional[torch.Tensor] = fa_tensor(convert=_semaphore_arg, default=None)
        mdK_semaphore: Optional[torch.Tensor] = fa_tensor(convert=_semaphore_arg, default=None)
        mdV_semaphore: Optional[torch.Tensor] = fa_tensor(convert=_semaphore_arg, default=None)
    ...
    def __init__(self, params: "FlashAttentionBackwardSm80.Params"):
        """Initializes the configuration for a flash attention v2 kernel.

        All contiguous dimensions must be at least 16 bytes aligned which indicates the head dimension
        should be a multiple of 8.

        :param params.head_dim: head dimension
        :type params.head_dim: int
        :param params.m_block_size: m block size
        :type params.m_block_size: int
        :param params.n_block_size: n block size
        :type params.n_block_size: int
        :param params.num_threads: number of threads
        :type params.num_threads: int
        :param params.is_causal: is causal
        """
        self.dtype = params.dtype
        # padding params.head_dim to a multiple of 32 (stricter than fwd's 16) due to
        ...
        self.num_stages_Q = params.num_stages_Q
        self.num_stages_dO = params.num_stages_dO
        self.SdP_swapAB = params.SdP_swapAB
        self.dKV_swapAB = params.dKV_swapAB
        self.dQ_swapAB = params.dQ_swapAB
        self.AtomLayoutMSdP = params.AtomLayoutMSdP
        self.AtomLayoutNdKV = params.AtomLayoutNdKV
        self.AtomLayoutMdQ = params.AtomLayoutMdQ
        num_mma_warps = self.num_threads // cute.arch.WARP_SIZE
        self.Mma_dKV_is_RS = params.AtomLayoutMSdP == 1 and params.AtomLayoutNdKV == num_mma_warps and params.SdP_swapAB and not params.dKV_swapAB
        self.V_in_regs = params.V_in_regs
        self.share_QV_smem = params.V_in_regs
        self.score_mod = params.score_mod
        self.score_mod_bwd = params.score_mod_bwd
    ...
    def __call__(
        ...
        mdQ_semaphore = args.mdQ_semaphore
        mdK_semaphore = args.mdK_semaphore
        mdV_semaphore = args.mdV_semaphore
