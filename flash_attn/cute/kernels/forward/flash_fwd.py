...
from flash_attn.cute.kernels.common.softmax import Softmax, apply_score_mod_inner
...
from flash_attn.cute.kernels.common.named_barrier import NamedBarrierFwd
...
class FlashAttentionForwardBase:
    ...
    class Params(KernelParams):
        ...
        Presence/dtype/rank of __call__ tensors is keyed mechanically via
        Args.spec(); score_mod/mask_mod are keyed by hash_callable in key().
        """
        ...
        pack_gqa: bool = sym_tag("pack_gqa", default=True)
        tile_m: int = sym_expr(lambda p: f"t{p.tile_m}x{p.tile_n}", default=128)
        tile_n: int = no_sym(default=128)
        num_stages: int = sym_val("st{}", skip=1, default=1)
        num_threads: int = sym_val("th{}", default=128)
        Q_in_regs: bool = sym_tag("q_regs", default=False)
    ...
    def __init__(self, params: "FlashAttentionForwardBase.Params"):
        ...
        :param params.tile_n: n block size
        :type params.tile_n: int
        :param params.num_threads: number of threads
        :type params.num_threads: int
        :param params.is_causal: is causal
        :param params.score_mod: A callable that takes the attention scores and applies a modification.
            Callable signature: ``params.score_mod(scores, batch_idx, head_idx, q_idx, kv_idx, aux_tensors) -> Any``
        :param params.mask_mod: A callable that takes the attention scores and returns a boolean representing whether that score should be masked.
            Callable signature: ``params.mask_mod(batch_idx, head_idx, q_idx, kv_idx, aux_tensors) -> Boolean``
        """
        self.dtype = params.dtype
        # padding params.head_dim to a multiple of 16 as k_block_size
        hdim_multiple_of = 16
        self.tile_hdim = int(math.ceil(params.head_dim / hdim_multiple_of) * hdim_multiple_of)
        ...
        self.check_hdim_oob = params.head_dim != self.tile_hdim
        ...
        self.Q_in_regs = params.Q_in_regs
        ...
        if self.score_vec_size > 2:
            raise ValueError(
                f"params.score_mod vec_size {self.score_vec_size} not supported on Sm80/90/120 "
        ...
        if self.mask_vec_size > 1:
            raise ValueError(
                f"params.mask_mod vec_size {self.mask_vec_size} not supported on Sm80/90/120 "
                "due to accumulator thread ownership pattern."
            )
