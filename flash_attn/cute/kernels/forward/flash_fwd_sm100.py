...
from flash_attn.cute.kernels.common.paged_kv import PagedKVManager
from flash_attn.cute.cute_dsl_utils import (
    assume_tensor_aligned,
    aux_data_call_arg,
    aux_data_compile_arg,
    aux_data_fingerprint,
    to_cute_tensor,
)
...
import flash_attn.cute.kernels.common.pipeline as pipeline_custom
import cutlass.pipeline as cutlass_pipeline
from flash_attn.cute.kernels.common.mask import AttentionMask
from flash_attn.cute.kernels.common.softmax import SoftmaxSm100, apply_score_mod_inner
from flash_attn.cute.kernels.common.seqlen_info import SeqlenInfoQK
from flash_attn.cute.kernels.common.block_info import BlockInfo
...
from flash_attn.cute.kernels.block_sparse.block_sparse_utils import (
    BlockSparseTensors,
...
from flash_attn.cute.kernels.common.pack_gqa import PackGQA, pack_gqa_layout
from flash_attn.cute.kernels.arch import mma_sm100_desc as sm100_desc
from flash_attn.cute.kernels.arch import blackwell_helpers as sm100_utils
from flash_attn.cute.kernels.common.named_barrier import NamedBarrierFwdSm100
...
# Composite-argument hooks for the Args declaration below.
def _descale_fingerprint(v: Optional[DescaleTensors]):
    if v is None:
        return None
    return tuple(t is not None for t in v)


def _descale_compile_arg(pool, v):
...
def _descale_call_arg(v):
    return v


def _hdim_sym(p) -> str:
    if p.head_dim_v in (None, p.head_dim):
        return f"d{p.head_dim}"
    return f"d{p.head_dim}x{p.head_dim_v}"
...
class FlashAttentionForwardSm100:
    ...
    class Params(KernelParams):
        ...
        This kernel takes no dtype: it is inferred from the tensors, and enters
        the cache key (and kernel symbol) through Args.spec().
        """
        ...
        is_split_kv: bool = sym_tag("splitkv", default=False)
        pack_gqa: bool = sym_tag("pack_gqa", default=False)
        q_subtile_factor: int = sym_val("qsub{}", skip=1, default=1)
        m_block_size: int = sym_expr(
            lambda p: f"t{p.m_block_size}x{p.n_block_size}", default=128
        )
        n_block_size: int = no_sym(default=128)
        q_stage: int = sym_val("qs{}", skip=1, default=2)
        is_persistent: bool = sym_tag("persistent", default=True)
        ...
        paged_kv_non_tma: bool = sym_tag("paged_non_tma", default=False)
        is_varlen_q: bool = no_sym(default=False)
        use_2cta_instrs: bool = sym_tag("2cta", default=False)
        use_clc_scheduler: bool = sym_tag("clc", default=False)

    @dataclass
    class Args(KernelArgs):
        """Runtime __call__ arguments: mirrors __call__ (minus stream) field-for-field."""

        mQ: torch.Tensor = fa_tensor(detach=True, fp8_as_uint8=True, sym_dtype=True)
        mK: torch.Tensor = fa_tensor(detach=True, fp8_as_uint8=True)
        mV: torch.Tensor = fa_tensor(detach=True, fp8_as_uint8=True)
        mO: torch.Tensor = fa_tensor(detach=True)
        mLSE: Optional[torch.Tensor] = fa_tensor(align=4, sym_absent="no_lse")
        softmax_scale: float = scalar(Float32)
        mCuSeqlensQ: Optional[torch.Tensor] = fa_tensor(
            align=4, leading_dim=0, sym="varlen", default=None
        )
        mCuSeqlensK: Optional[torch.Tensor] = fa_tensor(
            align=4, leading_dim=0, sym="varlen", default=None
        )
        mSeqUsedQ: Optional[torch.Tensor] = fa_tensor(
            align=4, leading_dim=0, sym="varlen", default=None
        )
        mSeqUsedK: Optional[torch.Tensor] = fa_tensor(
            align=4, leading_dim=0, sym="varlen", default=None
        )
        mPageTable: Optional[torch.Tensor] = fa_tensor(
            align=4, leading_dim=1, sym="paged", default=None
        )
        window_size_left: Optional[int] = scalar(Int32, default=None)
        window_size_right: Optional[int] = scalar(Int32, default=None)
        learnable_sink: Optional[torch.Tensor] = fa_tensor(
            align=4, leading_dim=0, sym="sink", default=None
        )
        descale_tensors: Optional[DescaleTensors] = composite(
            fingerprint=_descale_fingerprint,
            compile_build=_descale_compile_arg,
            call_build=_descale_call_arg,
            carrier_type=DescaleTensors,
            sym="descale",
            default=None,
        )
        blocksparse_tensors: Optional[BlockSparseTensorsTorch] = composite(
        ...
        aux_data: AuxData = composite(
            fingerprint=aux_data_fingerprint,
            compile_build=aux_data_compile_arg,
            call_build=aux_data_call_arg,
            carrier_type=AuxData,
            default=AuxData(),
        )

    def __init__(self, params: "FlashAttentionForwardSm100.Params"):
        self.use_tma_KV = not params.paged_kv_non_tma
        # self.dtype = dtype
        # padding params.head_dim to a multiple of 16 as k_block_size
        hdim_multiple_of = 16
        self.head_dim_padded = int(math.ceil(params.head_dim / hdim_multiple_of) * hdim_multiple_of)
        head_dim_v = params.head_dim_v if params.head_dim_v is not None else params.head_dim
        self.same_hdim_kv = params.head_dim == head_dim_v
        self.head_dim_v_padded = int(math.ceil(head_dim_v / hdim_multiple_of) * hdim_multiple_of)
        self.same_hdim_kv_padded = self.head_dim_padded == self.head_dim_v_padded
        self.check_hdim_oob = params.head_dim != self.head_dim_padded
        self.check_hdim_v_oob = head_dim_v != self.head_dim_v_padded
        self.m_block_size = params.m_block_size
        self.n_block_size = params.n_block_size
        self.q_stage = params.q_stage
        ...
        self.split_P_arrive = params.n_block_size // 4 * 3
        ...
        self.cta_tiler = (self.q_stage * params.m_block_size, params.n_block_size, self.head_dim_padded)
        # With 2CTA, the MMA tiler M covers both CTAs, so it's cta_group_size * params.m_block_size.
        # Each CTA owns params.m_block_size rows; the 2CTA MMA instruction spans both.
        self.mma_tiler_qk = (self.cta_group_size * params.m_block_size, params.n_block_size, self.head_dim_padded)
        self.mma_tiler_pv = (self.cta_group_size * params.m_block_size, self.head_dim_v_padded, params.n_block_size)
        self.qk_acc_dtype = Float32
        self.pv_acc_dtype = Float32
        self.cluster_shape_mn = (2, 1) if self.use_2cta_instrs else (1, 1)
        self.is_persistent = params.is_persistent
        self.is_causal = params.is_causal
        self.is_local = params.is_local
        self.is_varlen_q = params.is_varlen_q
        self.qhead_per_kvhead = params.qhead_per_kvhead
        self.is_split_kv = params.is_split_kv
        self.pack_gqa = params.pack_gqa
        self.use_tma_O = (
            not (self.pack_gqa and self.m_block_size % self.qhead_per_kvhead != 0)
            and not (self.pack_gqa and self.is_split_kv)
            and not params.is_varlen_q
        )
        self.use_correction_warps_for_epi = not self.use_tma_O
        self.q_subtile_factor = params.q_subtile_factor
        assert not (self.is_split_kv and self.head_dim_v_padded >= 192), (
            "SplitKV is not supported for hdim >= 192"
        )
        self.score_mod = params.score_mod
        self.mask_mod = params.mask_mod
        self.score_vec_size: cutlass.Constexpr = getattr(
            params.score_mod, "__vec_size__", 1 if cutlass.const_expr(params.has_aux_tensors) else 2
        )
        self.mask_vec_size: cutlass.Constexpr = getattr(params.mask_mod, "__vec_size__", 1)
        ...
        if self.head_dim_padded < 96:
            self.num_regs_softmax = 200 if not params.paged_kv_non_tma else 184
            self.num_regs_correction = 64
            self.num_regs_other = 48 if not params.paged_kv_non_tma else 80
        else:
            if not params.paged_kv_non_tma and "num_regs_softmax" in self._tune:
                self.num_regs_softmax = self._tune["num_regs_softmax"]
                self.num_regs_correction = self._tune["num_regs_correction"]
            elif not params.paged_kv_non_tma:
    ...
    def __call__(
        ...
        mQ = args.mQ  # (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
        mK = args.mK  # (b_k, s_k, h_k, d) or (total_k, h_k, d) if there is cu_seqlens_k or (num_pages, page_size, h_k, d) if there is page_table
        mV = args.mV  # (b_k, s_k, h_k, dv) or (total_k, h_k, dv) if there is cu_seqlens_k or (num_pages, page_size, h_k, dv) if there is page_table
        mO = args.mO  # (b, s_q, h, dv) or (total_q, h, dv) if there is cu_seqlens_q
        mLSE = args.mLSE
        ...
        mPageTable = args.mPageTable  # (b_k, max_num_pages_per_seq)
