...
from flash_attn.cute.kernels.common.mask import (
    Sm100FusedMask as FusedMask,
)
from flash_attn.cute.kernels.common.tile_scheduler import SM100_TMEM_CAPACITY_COLUMNS
from flash_attn.cute.kernels.forward.flash_fwd_sm100 import (
...
class BlackwellFusedMultiHeadAttentionForward:
    KERNEL_NAME = "flash_fwd_hd256"
    compile_mode = "real"
    call_convention = "args_struct"
    # __init__ and __call__ mirror FlashAttentionForwardSm100's signatures
    # exactly (enforced by check_kernel_contract), so the declarations are
    # shared rather than redeclared.
    Params = FlashAttentionForwardSm100.Params
    Args = FlashAttentionForwardSm100.Args

    def __init__(self, params: "BlackwellFusedMultiHeadAttentionForward.Params"):
        ...
        assert params.head_dim == 256 and head_dim_v == 256, (
            "SM100 dedicated kernel only supports (params.head_dim, head_dim_v) = (256, 256)"
        )
        assert params.score_mod is None, (
            "SM100 forward with head_dim=256 does not support params.score_mod"
        )
        assert params.mask_mod is None, (
            "SM100 forward with head_dim=256 does not support params.mask_mod"
        )
        assert not params.has_aux_tensors, (
            "SM100 forward with head_dim=256 does not support aux tensors"
        )
        assert not params.paged_kv_non_tma, (
            "SM100 hd256 2CTA supports TMA paged KV only (page_size must equal tile_n=128)"
        )
        assert not params.pack_gqa, (
            "SM100 forward with head_dim=256 does not support params.pack_gqa"
        )
        assert not params.is_split_kv, "SM100 forward with head_dim=256 does not support SplitKV"
        assert params.q_subtile_factor == 1, (
            "SM100 forward with head_dim=256 does not support params.q_subtile_factor"
        )
        assert params.m_block_size == 128 and params.n_block_size == 128, (
            "SM100 dedicated kernel only supports tile_m=128 and tile_n=128"
        )
        # params.q_stage / persistence / scheduler knobs are accepted for interface parity,
        # but this dedicated kernel uses fixed internal settings.

        qk_acc_dtype = cutlass.Float32
        pv_acc_dtype = cutlass.Float32
        mma_tiler = (128, 128, params.head_dim)
        ...
        self.use_semantic_trip_range = params.is_causal or params.is_local
        ...
        self.sched_warp_id = self.empty_warp_id[0] if params.use_clc_scheduler else None
        ...
        _tune_key = (True, params.is_causal, 256, False)  # hd256: always 2cta, no sm103 variant
