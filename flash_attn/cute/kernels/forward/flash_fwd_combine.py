...
def _is_varlen(args, params) -> bool:
    return args.cu_seqlens is not None


# 16-byte divisibility in elements of the *partial* dtype. NB: mO deliberately
# reuses it (transliterated from the original compile helper) — a weaker
# alignment assumption than mO could have.
def _partial_div(args, params) -> int:
    return 128 // (args.mO_partial.element_size() * 8)


class FlashAttentionForwardCombine:
    KERNEL_NAME = "flash_fwd_combine"
    ...
    class Params(KernelParams):
        ...
        Presence of each optional __call__ tensor is keyed mechanically via
        Args.spec(); dynamic dims (num_splits, batch, seqlen, nheads) are
        symbolic and never enter the key.
        """

        dtype: Type[cutlass.Numeric] = sym_val(short_dtype_name)
        dtype_partial: Type[cutlass.Numeric] = sym_val(
            lambda v: f"p{short_dtype_name(v)}", skip=Float32
        )
        head_dim: int = sym_val("d{}")
        tile_m: int = sym_val("m{}", default=8)
        k_block_size: int = sym_val("k{}", default=64)
        log_max_splits: int = sym_val(lambda v: f"sp{1 << v}", default=4)
        num_threads: int = sym_val("th{}", default=256)
        stages: int = sym_val("st{}", skip=4, default=4)

    @dataclass
    class Args(KernelArgs):
        """Runtime __call__ arguments: mirrors __call__ (minus stream) field-for-field.

        Shape templates mirror the runtime layouts: varlen packs (total_q,)
        where batched has (batch, seqlen); LSE tensors are seqlen-contiguous
        (produced via .transpose(-1, -2)). In the batched case "b" is shared
        between mO_partial and the per-batch 1-D tensors; in the varlen case it
        only appears in the 1-D tensors, so it is an independent symbol.
        """

        mO_partial: torch.Tensor = fa_tensor(
            divisibility=_partial_div,
            shape=lambda a, p: (
                ("splits", "total_q", "h", STATIC)
                if _is_varlen(a, p)
                else ("splits", "b", "s", "h", STATIC)
            ),
        )
        mLSE_partial: torch.Tensor = fa_tensor(
            align=4,
            shape=lambda a, p: (
                ("splits", "total_q", "h") if _is_varlen(a, p) else ("splits", "b", "s", "h")
            ),
            leading_dim=lambda a, p: 1 if _is_varlen(a, p) else 2,
        )
        mO: torch.Tensor = fa_tensor(
            divisibility=_partial_div,
            shape=lambda a, p: (
                ("total_q", "h", STATIC) if _is_varlen(a, p) else ("b", "s", "h", STATIC)
            ),
        )
        mLSE: Optional[torch.Tensor] = fa_tensor(
            align=4,
            shape=lambda a, p: ("total_q", "h") if _is_varlen(a, p) else ("b", "s", "h"),
            leading_dim=lambda a, p: 0 if _is_varlen(a, p) else 1,
            sym_absent="no_lse",
            default=None,
        )
        cu_seqlens: Optional[torch.Tensor] = fa_tensor(
            align=4, shape=("b_p1",), leading_dim=0, sym="varlen", default=None
        )
        seqused: Optional[torch.Tensor] = fa_tensor(
            align=4, shape=("b",), leading_dim=0, sym="seqused", default=None
        )
        num_splits_dynamic_ptr: Optional[torch.Tensor] = fa_tensor(
            align=4, shape=("b",), leading_dim=0, sym="dynsplits", default=None
        )
        varlen_batch_idx: Optional[torch.Tensor] = fa_tensor(
            align=4, shape=("b",), leading_dim=0, sym="batchidx", default=None
        )
        semaphore_to_reset: Optional[torch.Tensor] = fa_tensor(
            align=4, shape=("sem",), leading_dim=0, sym="sem", default=None
        )

    @classmethod
    def can_implement_params(cls, params: "FlashAttentionForwardCombine.Params") -> Optional[str]:
        ok = cls.can_implement(
            params.dtype,
            params.dtype_partial,
            params.head_dim,
            params.tile_m,
            params.k_block_size,
            params.log_max_splits,
            params.num_threads,
        )
        return None if ok else "unsupported dtype/head_dim/tile/split configuration"

    def __init__(self, params: "FlashAttentionForwardCombine.Params"):
        """
        Forward combine kernel for split attention computation.

        :param params.dtype: output data type
        :param params.dtype_partial: partial accumulation data type
        :param params.head_dim: head dimension
        :param params.tile_m: m block size
        :param params.k_block_size: k block size
        :param params.log_max_splits: log2 of maximum splits
        :param params.num_threads: number of threads
        :param varlen: whether using variable length sequences
        :param params.stages: number of pipeline params.stages
        """
        self.dtype = params.dtype
        self.dtype_partial = params.dtype_partial
        self.head_dim = params.head_dim
        self.tile_m = params.tile_m
        self.k_block_size = params.k_block_size
        self.max_splits = 1 << params.log_max_splits
        self.num_threads = params.num_threads
        self.is_even_k = params.head_dim % params.k_block_size == 0
    ...
    def __call__(
        ...
        mO_partial = args.mO_partial
        mLSE_partial = args.mLSE_partial
        mO = args.mO
        mLSE = args.mLSE
        cu_seqlens = args.cu_seqlens
        seqused = args.seqused
        num_splits_dynamic_ptr = args.num_splits_dynamic_ptr
        varlen_batch_idx = args.varlen_batch_idx
        semaphore_to_reset = args.semaphore_to_reset
        # Type checking
