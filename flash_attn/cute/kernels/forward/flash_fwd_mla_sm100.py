...
from flash_attn.cute.kernels.common.pack_gqa import pack_gqa_layout, make_packgqa_tiled_tma_atom
...
import flash_attn.cute.kernels.arch.blackwell_helpers as fa_sm100_utils
from flash_attn.cute.kernels.common.softmax import SoftmaxSm100
...
from flash_attn.cute.kernels.common.topk_gather_kv import CpasyncGatherKVManager
...
from flash_attn.cute.kernels.common.named_barrier import NamedBarrierFwdSm100_MLA2CTA


class FlashAttentionMLAForwardSm100:
    KERNEL_NAME = "flash_fwd_mla"
    compile_mode = "real"
    ...
    class Params(KernelParams):
        ...
        use_cpasync_load_KV: bool = sym_tag("cpasync_kv", default=False)
        topk_length: int = sym_val("topk{}", default=2048)
        is_topk_gather: bool = sym_tag("topk_gather", default=True)
        pack_gqa: bool = sym_tag("pack_gqa", default=False)
        qhead_per_kvhead: int = sym_val("gqa{}", skip=1, default=1)
        nheads_kv: int = sym_val("hkv{}", skip=1, default=1)
        hdim: int = sym_val("d{}", default=64)
        hdimv: int = sym_val("dv{}", default=512)
        has_seqused_q: bool = no_sym(default=False)
        has_cu_seqlens_q: bool = no_sym(default=False)
        disable_bitmask: bool = sym_tag("no_bitmask", default=False)
        use_clc_scheduler: bool = sym_tag("clc", default=True)
        has_qk: bool = sym_tag("noqk", invert=True, default=True)

    @dataclass
    class Args(KernelArgs):
        """Runtime __call__ arguments: mirrors __call__ (minus stream) field-for-field."""

        mQ: Optional[torch.Tensor] = fa_tensor(detach=True, fp8_as_uint8=True)
        mQv: torch.Tensor = fa_tensor(detach=True, fp8_as_uint8=True, sym_dtype=True)
        mK: Optional[torch.Tensor] = fa_tensor(detach=True, fp8_as_uint8=True)
        ...
        mP: Optional[torch.Tensor] = fa_tensor(sym="p", default=None)
        mRowMax: Optional[torch.Tensor] = fa_tensor(sym="rowmax", default=None)
        ...
        mIndexTopk: Optional[torch.Tensor] = fa_tensor(default=None)
    ...
    def __init__(self, params: "FlashAttentionMLAForwardSm100.Params"):
        self.is_causal = params.is_causal
        self.is_local = False
        self.pack_gqa = params.pack_gqa
        self.qhead_per_kvhead = params.qhead_per_kvhead
        assert params.qhead_per_kvhead <= 128
        self.nheads_kv = params.nheads_kv
        self.use_tma_O = True
        self.use_cpasync_load_KV = params.use_cpasync_load_KV
        self.use_tma_KV = not params.use_cpasync_load_KV
        self.topk_length = params.topk_length
        self.is_topk_gather = params.is_topk_gather
        if params.is_topk_gather:
            assert params.pack_gqa
            assert params.qhead_per_kvhead == 128, "require MQA 128 for DSA path"
            assert params.use_cpasync_load_KV
        # user-provided option if topk indices guaranteed in bounds
        self.disable_bitmask = params.disable_bitmask
        self.has_qk = params.has_qk
        ...
        self.is_varlen_q = params.has_seqused_q or params.has_cu_seqlens_q
        self.use_packed_varlen_sched = (
            params.has_cu_seqlens_q and params.qhead_per_kvhead == 128 and params.pack_gqa
        )
        ...
        self.num_empty_threads = 32 if params.use_cpasync_load_KV else 64
        self.num_relay_threads = 32 if params.use_cpasync_load_KV else 0
        self.num_cpasync_load_threads = 128 if params.use_cpasync_load_KV else 0
        self.num_threads = (
        ...
        self.empty_warp_ids = tuple(
            w
            for w, active in [
                (self.relay_warp_id, not params.use_cpasync_load_KV),
        ...
        self.hdimv = params.hdimv
        self.cta_tile_m = 64
        self.cluster_tile_m = self.cta_group_size * self.cta_tile_m
        self.tile_n = 128
        assert (
            params.pack_gqa is False
            or self.cluster_tile_m % params.qhead_per_kvhead == 0
            or params.qhead_per_kvhead % self.cluster_tile_m == 0
        )
        self.num_hdimv_splits = 2  # split params.hdimv in half for our Qv @ V^T and P @ V mmas.
        assert params.hdimv % 32 == 0
        ...
        self.num_stages_V = 4
        self.num_stages_S = 2
        # self.num_stages_P = 1 if params.has_qk else 2
        self.num_stages_P = 1
    ...
    def __call__(
        ...
        mQv = args.mQv  # (b, s_q, h, dv)    or (total_q, h, d)    if there is cu_seqlens_q
        mK = args.mK  # (b, s_k, h_k, d)   or (total_k, h_k, d)  if there is cu_seqlens_k  or (num_pages, page_size, h_k, d)  if there is page_table
        mV = args.mV  # (b, s_k, h_k, dv)  or (total_k, h_k, dv) if there is cu_seqlens_k  or (num_pages, page_size, h_k, dv) if there is page_table
        mO = args.mO  # (b, s_q, h, dv)    or (total_q, h, dv)   if there is cu_seqlens_q
        mLSE = args.mLSE  # (b, s_q, h)        or (total_q, h)       if there is cu_seqlens_q
        softmax_scale = args.softmax_scale
        mP = args.mP  # (b, s_q, h, topk)            or (total_q, h, topk)           if there is cu_seqlens_q
        mRowMax = args.mRowMax  # (b, s_q, topk // tile_n, h)  or (total_q, topk // tile_n, h) if there is cu_seqlens_q
        mCuSeqlensQ = args.mCuSeqlensQ  # (b + 1)
        mCuSeqlensK = args.mCuSeqlensK  # (b + 1)
        mSeqUsedQ = args.mSeqUsedQ  # (b)
        mSeqUsedK = args.mSeqUsedK  # (b)
        mIndexTopk = args.mIndexTopk  # (b, s_q, topk)  or (total_q, topk) if there is cu_seqlens_q
